...
static void UpdateSoftBits(unsigned char *data, int bitstart, int bitlen, int weight) {
    ...
    for (i = 0; i < bitlen; i++) {
        ...
        if (data[i / 8] & bitmask[i % 8]) fdata[i + bitstart] += weight;
        else
            fdata[i + bitstart] -= weight; //updates soft bits depends metric
    }
}
...
static void AssembleSoftBits(unsigned char *data, int bitstart, int bitlen) {
    ...
    for (i = 0; i < bitlen; i++)
        if (fdata[i + bitstart] > 0)
            data[i / 8] |= bitmask[i % 8]; //hard decision of soft bits
}
...
//    printf("zzzzzzzzzzzzzzzzzzzzzzzz here 6 %lu \r\n", sizeof pkt);
    ...
    if (cnt_out ==
        7)  //type7: checksum (algo 1) of 224 bits of our session (ephemeral) public key transmitted on DH stage
    {
...
static int ProcessDH(unsigned char *pkt) {
    ...
    if ((step == 1) || (ptype == 8))
        UpdateSoftBits(udata, 32 * ptype, 32, w); //accumulate received bits while whole key received
...
static int MakeAU(unsigned char *pkt) {
    ...
    if (cnt_out == 8)
        d = 0xFFFFFFFF ^ crc32(0, pubkey,
                               28); //type8: crc0 of our pubkey (for finalize key sequence on the remote side)
